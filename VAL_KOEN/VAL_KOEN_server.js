'use strict';
/**
 * VectraArch Legacy — server.js (Fully Dynamic Engine Instance)
...
// Dynamically anchor to the local directory containing this running instance
...
// Abstract configuration profiles with flexible execution defaults
const PORT = process.env.PORT || 8444;
const PFX  = process.env.DB_PREFIX || 'VAL_';

// ── POSTGRESQL CONNECTION CLUSTER ──
const pool = new Pool({
...
pool.connect((err, client, release) => {
    if (err) { 
        console.error(`Failed to connect to unique ${process.env.DB_NAME || 'target'} cluster:`, err.message); 
        return; 
    }
    release();
    console.log(`Connected natively to ${process.env.DB_NAME} (PostgreSQL Engine Instance).`);
});

// ── DATABASE ROUTING CORE HELPERS ──
...
// ── COLUMN NAME DATA-STRUCTURE MAP ──
...
async function logTransaction(username, action, tableName, recordId, modifiedBy) {
    try {
        await dbRun(
            `INSERT INTO "${PFX}transaction_history" (username, action, table_name, record_id, modified_by, modified_at) VALUES ($1,$2,$3,$4,$5,$6)`,
...
// ── CUSTOM SECURE ROUTE VALIDATION MIDDLEWARE ──
const requireAdmin = async (req, res, next) => {
    ...
    try {
        const row = await dbQuery(`SELECT is_admin FROM "${PFX}users" WHERE username = $1`, [adminUsername]);
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        const row = await dbQuery(`SELECT * FROM "${PFX}users" WHERE username = $1`, [username]);
        ...
        await dbRun(`UPDATE "${PFX}users" SET last_active = $1 WHERE username = $2`, [new Date().toISOString(), username]);
...
// ── SECURITY SYSTEM USERS INTERFACES ──
app.get('/api/users', requireAdmin, async (req, res) => {
    try {
        const rows = await dbAll(
            `SELECT username, first_name AS "firstName", last_name AS "lastName", display_name AS "displayName", is_admin AS "isAdmin", activity_status AS "activityStatus", last_active AS "lastActive" FROM "${PFX}users"`
        );
...
app.post('/api/add-user', requireAdmin, async (req, res) => {
    ...
    if (!username || !password) return res.status(400).json({ success: false, message: 'Username and password required.' });
    try {
        const existing = await dbQuery(`SELECT username FROM "${PFX}users" WHERE username = $1`, [username]);
        if (existing) return res.status(400).json({ success: false, message: 'User already exists.' });
        const hash = await bcrypt.hash(password, 10);
        await dbRun(
            `INSERT INTO "${PFX}users" (username, password_hash, first_name, last_name, display_name, is_admin) VALUES ($1,$2,$3,$4,$5,0)`,
...
app.delete('/api/delete-user/:username', requireAdmin, async (req, res) => {
    ...
    try {
        const row = await dbQuery(`SELECT username FROM "${PFX}users" WHERE username = $1`, [username]);
        if (!row) return res.status(404).json({ success: false, message: 'User not found.' });
        await dbRun(`DELETE FROM "${PFX}users" WHERE username = $1`, [username]);
...
app.post('/api/grant-admin', requireAdmin, async (req, res) => {
    ...
    if (!username) return res.status(400).json({ success: false, message: 'Username required.' });
    try {
        const row = await dbQuery(`SELECT is_admin FROM "${PFX}users" WHERE username = $1`, [username]);
        if (!row) return res.status(404).json({ success: false, message: 'User not found.' });
        if (row.is_admin) return res.status(400).json({ success: false, message: 'User is already an admin.' });
        await dbRun(`UPDATE "${PFX}users" SET is_admin = 1 WHERE username = $1`, [username]);
...
app.post('/api/revoke-admin', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        await dbRun(`UPDATE "${PFX}users" SET is_admin = 0 WHERE username = $1`, [username]);
...
app.post('/api/admin-update-password', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        await dbRun(`UPDATE "${PFX}users" SET password_hash = $1 WHERE username = $2`, [hash, username]);
...
app.post('/api/update-password', async (req, res) => {
    ...
    try {
        const row = await dbQuery(`SELECT password_hash FROM "${PFX}users" WHERE username = $1`, [username]);
        if (!row) return res.status(404).json({ success: false, message: 'User not found.' });
...
// ── USER INSTANCE METADATA AND PROFILES ──
...
app.post('/api/profile-pictures', async (req, res) => {
    ...
    try {
        const exists = await dbQuery(`SELECT username FROM "${PFX}users" WHERE username = $1`, [username]);
        if (!exists) return res.status(404).json({ success: false, message: 'User not found.' });
        await dbRun(`
            UPDATE "${PFX}users" SET
        ...
        const updated = await dbQuery(`SELECT * FROM "${PFX}users" WHERE username = $1`, [username]);
...
app.get('/api/user-color', async (req, res) => {
    ...
    if (!username) return res.status(400).json({ success: false, message: 'Username required.' });
    try {
        const row = await dbQuery(`SELECT event_color FROM "${PFX}users" WHERE username = $1`, [username]);
        if (!row) return res.status(404).json({ success: false, message: 'User not found.' });
...
// ── SECURITY EDGE CROSS-ACCESS CONTROLS ──
app.get('/api/get-access', async (req, res) => {
    const { viewer, adminUsername } = req.query;
    try {
        let rows;
        if (adminUsername) {
            const admin = await dbQuery(`SELECT is_admin FROM "${PFX}users" WHERE username = $1`, [adminUsername]);
            if (admin && admin.is_admin) {
                rows = await dbAll(`SELECT viewer, target FROM "${PFX}access"`);
            } else {
                if (!viewer) return res.status(400).json({ success: false, message: 'Viewer required if not admin.' });
                rows = await dbAll(`SELECT viewer, target FROM "${PFX}access" WHERE viewer = $1`, [viewer]);
            }
        } else {
...
app.post('/api/grant-access', requireAdmin, async (req, res) => {
    ...
    try {
        const vRow = await dbQuery(`SELECT username FROM "${PFX}users" WHERE username = $1`, [viewer]);
        if (!vRow) return res.status(404).json({ success: false, message: 'Viewer not found.' });
        const tRow = await dbQuery(`SELECT username FROM "${PFX}users" WHERE username = $1`, [target]);
        if (!tRow) return res.status(404).json({ success: false, message: 'Target not found.' });
        const ab = await dbQuery(`SELECT 1 FROM "${PFX}access" WHERE viewer=$1 AND target=$2`, [viewer, target]);
        const ba = await dbQuery(`SELECT 1 FROM "${PFX}access" WHERE viewer=$1 AND target=$2`, [target, viewer]);
        if (ab && ba) return res.status(400).json({ success: false, message: 'Access already shared.' });
        const queries = [];
        if (!ab) queries.push({ sql: `INSERT INTO "${PFX}access" (viewer,target) VALUES ($1,$2) ON CONFLICT DO NOTHING`, params: [viewer, target] });
        if (!ba) queries.push({ sql: `INSERT INTO "${PFX}access" (viewer,target) VALUES ($1,$2) ON CONFLICT DO NOTHING`, params: [target, viewer] });
        queries.push({ sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5)`, params: [viewer,'GRANT_ACCESS','access',req.adminUsername,new Date().toISOString()] });
        queries.push({ sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5)`, params: [target,'GRANT_ACCESS','access',req.adminUsername,new Date().toISOString()] });
...
app.post('/api/revoke-access', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const queries = [];
        if (ab) queries.push({ sql: `DELETE FROM "${PFX}access" WHERE viewer=$1 AND target=$2`, params: [viewer, target] });
        if (ba) queries.push({ sql: `DELETE FROM "${PFX}access" WHERE viewer=$1 AND target=$2`, params: [target, viewer] });
        queries.push({ sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5)`, params: [viewer,'REVOKE_ACCESS','access',req.adminUsername,new Date().toISOString()] });
        queries.push({ sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5)`, params: [target,'REVOKE_ACCESS','access',req.adminUsername,new Date().toISOString()] });
        await dbTransaction(queries);
...
// ── ALERTS AND NOTIFICATION METRICS ──
app.get('/api/notifications', async (req, res) => {
    const { username } = req.query;
    if (!username) return res.status(400).json({ success: false, message: 'Username required.' });
    try {
        const rows = await dbAll(`SELECT type, enabled FROM "${PFX}notifications" WHERE username = $1`, [username]);
...
app.post('/api/notifications', async (req, res) => {
    ...
    try {
        await dbRun(
            `INSERT INTO "${PFX}notifications" (username,type,enabled) VALUES ($1,$2,$3) ON CONFLICT (username,type) DO UPDATE SET enabled=EXCLUDED.enabled`,
...
// ── AUDITING LAYER TRANSACTIONS ──
app.get('/api/transaction-history', async (req, res) => {
    const { username } = req.query;
    if (!username) return res.status(400).json({ success: false, message: 'Username required.' });
    try {
        const rows = await dbAll(
            `SELECT id, action, table_name AS "tableName", record_id AS "recordId", modified_by AS "modifiedBy", modified_at AS "modifiedAt" FROM "${PFX}transaction_history" WHERE username = $1 ORDER BY modified_at DESC`,
            [username]
        );
...
// ── NATIVE FINANCIAL SERVICES ROUTING ──
app.get('/api/financial', async (req, res) => {
    const { user } = req.query;
    if (!user) return res.status(400).json({ success: false, message: 'User required.' });
    try {
        const rows = await dbAll(`SELECT id, username, category, amount, type, TO_CHAR(date, 'YYYY-MM-DD') AS date FROM "${PFX}financial" WHERE username = $1`, [user]);
        res.json(rows);
    } catch (e) {
...
app.post('/api/financial', async (req, res) => {
    ...
    try {
        const fRes = await dbRun(
            `INSERT INTO "${PFX}financial" (username,category,amount,type,date) VALUES ($1,$2,$3,$4,$5) RETURNING id`,
            [user, category, amount, type, date]
        );
        await dbRun(
            `INSERT INTO "${PFX}calendar" (username,title,date,is_financial,type,amount,event_color) VALUES ($1,$2,$3,1,$4,$5,$6)`,
        ...
        const userData = await dbQuery(`SELECT telegram_chat_id, email FROM "${PFX}users" WHERE username = $1`, [user]);
        const notifs = await dbAll(`SELECT type, enabled FROM "${PFX}notifications" WHERE username = $1`, [user]);
...
app.put('/api/financial/:id', async (req, res) => {
    ...
    if (isNaN(amount) || amount < 0) return res.status(400).json({ success: false, message: 'Amount must be a non-negative number.' });
    try {
        const row = await dbQuery(`SELECT id FROM "${PFX}financial" WHERE id = $1 AND username = $2`, [id, user]);
        if (!row) return res.status(404).json({ success: false, message: 'Financial item not found.' });
        await dbTransaction([
            { sql: `UPDATE "${PFX}financial" SET category=$1,amount=$2,type=$3,date=$4 WHERE id=$5`, params: [category,amount,type,date,id] },
            { sql: `UPDATE "${PFX}calendar" SET title=$1,date=$2,type=$3,amount=$4 WHERE is_financial=1 AND username=$5 AND amount=(SELECT amount FROM "${PFX}financial" WHERE id=$6)`, params: [`${category} (${type})`,date,type,amount,user,id] },
            { sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,record_id,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5,$6)`, params: [user,'UPDATE','financial',id,user,new Date().toISOString()] }
        ]);
        const userData = await dbQuery(`SELECT telegram_chat_id, email FROM "${PFX}users" WHERE username = $1`, [user]);
        const notifs = await dbAll(`SELECT type, enabled FROM "${PFX}notifications" WHERE username = $1`, [user]);
...
app.delete('/api/financial/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const row = await dbQuery(`SELECT id, username, amount, type, date, category FROM "${PFX}financial" WHERE id = $1`, [id]);
        if (!row) return res.status(404).json({ success: false, message: 'Financial item not found.' });
        await dbTransaction([
            { sql: `DELETE FROM "${PFX}financial" WHERE id=$1`, params: [id] },
            { sql: `DELETE FROM "${PFX}calendar" WHERE is_financial=1 AND username=$1 AND amount=$2 AND type=$3 AND date=$4`, params: [row.username,row.amount,row.type,row.date] },
            { sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,record_id,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5,$6)`, params: [row.username,'DELETE','financial',id,row.username,new Date().toISOString()] }
        ]);
        const userData = await dbQuery(`SELECT telegram_chat_id, email FROM "${PFX}users" WHERE username = $1`, [row.username]);
        const notifs = await dbAll(`SELECT type, enabled FROM "${PFX}notifications" WHERE username = $1`, [row.username]);
...
// ── METRIC PLANNED BUDGET ENGINE ──
app.get('/api/budget', async (req, res) => {
    const { user } = req.query;
    if (!user) return res.status(400).json({ success: false, message: 'User required.' });
    try {
        const rows = await dbAll(
            `SELECT id, username AS "user", income, expenses, TO_CHAR(date, 'YYYY-MM-DD') AS date, COALESCE(budget_type,'need') AS budget_type FROM "${PFX}budget" WHERE username = $1 ORDER BY date DESC`,
            [user]
        );
...
app.post('/api/budget', async (req, res) => {
    ...
    try {
        const r = await dbRun(
            `INSERT INTO "${PFX}budget" (username,income,expenses,date,budget_type) VALUES ($1,$2,$3,$4,$5) RETURNING id`,
...
app.put('/api/budget/:id', async (req, res) => {
    ...
    const dateVal = String(date).slice(0, 10);
    try {
        const row = await dbQuery(`SELECT id FROM "${PFX}budget" WHERE id=$1 AND username=$2`, [id, user]);
        if (!row) return res.status(404).json({ success: false, message: 'Budget not found.' });
        await dbTransaction([
            { sql: `UPDATE "${PFX}budget" SET income=$1,expenses=$2,date=$3,budget_type=$4 WHERE id=$5`, params:[incomeVal,expArr,dateVal,budType,id] },
            { sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,record_id,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5,$6)`, params:[user,'UPDATE','budget',id,user,new Date().toISOString()] }
        ]);
...
app.delete('/api/budget/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const row = await dbQuery(`SELECT id, username FROM "${PFX}budget" WHERE id=$1`, [id]);
        if (!row) return res.status(404).json({ success: false, message: 'Budget not found.' });
        await dbTransaction([
            { sql: `DELETE FROM "${PFX}budget" WHERE id=$1`, params: [id] },
            { sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,record_id,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5,$6)`, params: [row.username,'DELETE','budget',id,row.username,new Date().toISOString()] }
        ]);
...
// ── DYNAMIC CALENDAR ROUTER ──
app.get('/api/calendar', async (req, res) => {
    const { user } = req.query;
    if (!user) return res.status(400).json({ success: false, message: 'User required.' });
    try {
        const rows = await dbAll(
            `SELECT id, username AS user, title, TO_CHAR(date, 'YYYY-MM-DD HH24:MI:SS') AS date, is_financial AS financial, type, amount, event_color AS "eventColor" FROM "${PFX}calendar" WHERE username = $1`,
            [user]
        );
        res.json(rows);
    } catch (e) {
...
app.post('/api/calendar', async (req, res) => {
    ...
    try {
        ...
        const r = await dbRun(
            `INSERT INTO "${PFX}calendar" (username,title,date,is_financial,type,amount,event_color) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
...
app.delete('/api/calendar/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const row = await dbQuery(`SELECT id, username, title, is_financial, amount, type, date FROM "${PFX}calendar" WHERE id=$1`, [id]);
        if (!row) return res.status(404).json({ success: false, message: 'Calendar event not found.' });
        const queries = [
            { sql: `DELETE FROM "${PFX}calendar" WHERE id=$1`, params: [id] },
            { sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,record_id,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5,$6)`, params: [row.username,'DELETE','calendar',id,row.username,new Date().toISOString()] }
        ];
        if (row.is_financial && row.amount) {
            queries.push({
                sql: `DELETE FROM "${PFX}financial" WHERE username=$1 AND category=$2 AND amount=$3 AND type=$4 AND TO_CHAR(date,'YYYY-MM-DD')=TO_CHAR($5::date,'YYYY-MM-DD')`,
...
app.get('/api/gymworkout', async (req, res) => {
    ...
    if (!user) return res.status(400).json({ success: false, message: 'User required.' });
    try {
        const rows = await dbAll(`SELECT id, username AS user, day, exercise, sets, reps, weight, date FROM "${PFX}gymworkout" WHERE username = $1`, [user]);
        res.json(rows);
    } catch (e) {
...
app.post('/api/gymworkout', async (req, res) => {
    ...
    try {
        const r = await dbRun(
            `INSERT INTO "${PFX}gymworkout" (username,day,exercise,sets,reps,weight,date) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id`,
...
app.put('/api/gymworkout/:id', async (req, res) => {
    ...
    if (!user || !day || !exercise || !sets || !reps || !weight || !date) return res.status(400).json({ success: false, message: 'All fields required.' });
    try {
        const row = await dbQuery(`SELECT id FROM "${PFX}gymworkout" WHERE id=$1 AND username=$2`, [id, user]);
        if (!row) return res.status(404).json({ success: false, message: 'Workout not found.' });
        await dbTransaction([
            { sql: `UPDATE "${PFX}gymworkout" SET day=$1,exercise=$2,sets=$3,reps=$4,weight=$5,date=$6 WHERE id=$7`, params: [day, exercise, parseInt(sets)||null, String(reps), String(weight), date, id] },
            { sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,record_id,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5,$6)`, params: [user,'UPDATE','gymworkout',id,user,new Date().toISOString()] }
        ]);
...
app.delete('/api/gymworkout/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const row = await dbQuery(`SELECT id, username FROM "${PFX}gymworkout" WHERE id=$1`, [id]);
        if (!row) return res.status(404).json({ success: false, message: 'Gym workout not found.' });
        await dbTransaction([
            { sql: `DELETE FROM "${PFX}gymworkout" WHERE id=$1`, params: [id] },
            { sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,record_id,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5,$6)`, params: [row.username,'DELETE','gymworkout',id,row.username,new Date().toISOString()] }
        ]);
...
// ── NUTRITION AND MEAL PLAN CONTROLS ──
...
app.get('/api/mealplan', async (req, res) => {
    ...
    try {
        const rows = await dbAll(
            `SELECT id, username AS user, day, meal_type AS "mealType", description, calories, date FROM "${PFX}mealplan" WHERE username = $1`,
            [user]
        );
        res.json(rows);
    } catch (e) {
...
app.post('/api/mealplan', async (req, res) => {
    ...
    try {
        const r = await dbRun(
            `INSERT INTO "${PFX}mealplan" (username,day,meal_type,description,calories,date) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`,
...
app.delete('/api/mealplan/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const row = await dbQuery(`SELECT id, username FROM "${PFX}mealplan" WHERE id=$1`, [id]);
        if (!row) return res.status(404).json({ success: false, message: 'Meal plan not found.' });
        await dbTransaction([
            { sql: `DELETE FROM "${PFX}mealplan" WHERE id=$1`, params: [id] },
            { sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,record_id,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5,$6)`, params: [row.username,'DELETE','mealplan',id,row.username,new Date().toISOString()] }
        ]);
...
// ── BIOMETRIC CYCLE TRACKING ENGINE ──
app.get('/api/period', async (req, res) => {
    const { user } = req.query;
    if (!user) return res.status(400).json({ success: false, message: 'User required.' });
    try {
        const rows = await dbAll(
            `SELECT id, username AS user, start_date AS "startDate", end_date AS "endDate", cycle_length AS "cycleLength", symptoms, date FROM "${PFX}period" WHERE username = $1`,
            [user]
        );
        res.json(rows);
    } catch (e) {
...
app.post('/api/period', async (req, res) => {
    ...
    try {
        const r = await dbRun(
            `INSERT INTO "${PFX}period" (username,start_date,end_date,cycle_length,symptoms,date) VALUES ($1,$2,$3,$4,$5,$6) RETURNING id`,
...
app.delete('/api/period/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const row = await dbQuery(`SELECT id, username FROM "${PFX}period" WHERE id=$1`, [id]);
        if (!row) return res.status(404).json({ success: false, message: 'Period cycle not found.' });
        await dbTransaction([
            { sql: `DELETE FROM "${PFX}period" WHERE id=$1`, params: [id] },
            { sql: `INSERT INTO "${PFX}transaction_history" (username,action,table_name,record_id,modified_by,modified_at) VALUES ($1,$2,$3,$4,$5,$6)`, params: [row.username,'DELETE','period',id,row.username,new Date().toISOString()] }
        ]);
...
// ── TRANSACTION BANK STATEMENT EXTRACTOR ENGINE ──
...
// ── SECURE TWO-FACTOR AUTHENTICATION ENGINE ──
app.get('/api/2fa/status', async (req, res) => {
    const { username } = req.query;
    if (!username) return res.status(400).json({ success: false, message: 'Username required.' });
    try {
        const row = await dbQuery(`SELECT twofa_secret FROM "${PFX}users" WHERE username = $1`, [username]);
        if (!row) return res.status(404).json({ success: false, message: 'User not found.' });
...
app.post('/api/2fa/setup', async (req, res) => {
    ...
    try {
        ...
        await dbRun(`UPDATE "${PFX}users" SET twofa_secret=$1 WHERE username=$2`, [secret, username]);
...
app.post('/api/2fa/disable', async (req, res) => {
    ...
    try {
        ...
        await dbRun(`UPDATE "${PFX}users" SET twofa_secret=NULL WHERE username=$1`, [username]);
...
// ── VECTRA ARCH IDENTITY PROXY GATEWAY ──
const IDENTITY_URL  = process.env.IDENTITY_URL || 'http://127.0.0.1:3200';
...
// ── SECURE NETWORK NETWORK HANDSHAKE DAEMON ──
const httpsOptions = {
...
httpsServer.listen(PORT, () => console.log(`HTTPS Secure Server running on port ${PORT} with prefix: ${PFX}`));
